...
def modify_request(body):
	while True:   
		...
		siid = key_list[index]
		if (len(sp_dict[siid]) != 0):
			piid = sp_dict.get(siid)[random.randint(0,len(sp_dict.get(siid))-1)]
			if len(siid_piid_types[key_list.index(siid)][sp_dict[siid].index(piid)]) != 0:
				break
	types_tmp = siid_piid_types[key_list.index(siid)][sp_dict[siid].index(piid)]
	arrays = [True, False]
	v = RandomValues()
	empty_method = lambda *kargs, **kwargs: None
	i = random.randint(0,len(types_tmp) - 1)
	t = types_tmp[i]
	...
	# body_new = body_new.replace("539689906", "1071189748")
	...
	ssecurity = "Dv/1y7SRAO4inmERfqxZ6A=="
	uri = "/miotspec/prop/set"
	sig = sign_data(uri, body_new, ssecurity)
	global siid_piid_after 
	siid_piid_after = [key_list.index(siid), sp_dict[siid].index(piid), i]
	return body_new, sig
